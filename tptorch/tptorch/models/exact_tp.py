...
class ExactTP(GP):
    r"""
    The base class for any student's T process latent function to be used in conjunction
    ...
    :param ~StudentTLikelihood likelihood: The Student's T likelihood that defines
        the observational distribution. Since we're using exact inference, the likelihood must be StudentT.

    The :meth:`forward` function should describe how to compute the prior latent distribution
    on a given input. Typically, this will involve a mean and kernel function.
    The result must be a :obj:`~MultivariateStudentT`.

    Calling this model will return the posterior of the latent T process when conditioned
    on the training data. The output will be a :obj:`~MultivariateStudentT`.
    ...
    def __init__(self, train_inputs, train_targets, likelihood):
        ...
        if train_inputs is not None:
            self.train_inputs = tuple(
                tri.unsqueeze(-1) if tri.ndimension() == 1 else tri for tri in train_inputs
            )
    ...
    def set_train_data(self, inputs=None, targets=None, strict=True):
        ...
        if inputs is not None:
            if torch.is_tensor(inputs):
                inputs = (inputs,)
            inputs = tuple(
                input_.unsqueeze(-1) if input_.ndimension() == 1 else input_ for input_ in inputs
            )
            if strict:
        ...
        if targets is not None:
            if strict:
                for attr in {"shape", "dtype", "device"}:
                    ...
                    if expected_attr != found_attr:
                        msg = (
                            "Cannot modify {attr} of targets (expected {e_attr}, found {f_attr})."
                        )
                        msg = msg.format(attr=attr, e_attr=expected_attr, f_attr=found_attr)
                        raise RuntimeError(msg)
    ...
    def train(self, mode=True):
        if mode:
            self.prediction_strategy = None
        return super(ExactTP, self).train(mode)
